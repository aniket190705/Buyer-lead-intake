...
export default function EditBuyerForm({ buyer }: EditBuyerFormProps) {
  ...
  try {
    ...
    if (response.ok) {
      toast.success('Lead updated successfully!') // ✅ Should work now
      router.push(`/buyers/${buyer.id}`)
    } else {
      if (response.status === 403) {
        toast.error('❌ You are not authorized to edit this lead.')
  ...
  const handleDelete = async () => {
    ...
    const deleteToast = toast.loading('Deleting lead...') // ✅ Loading toast
    ...
    try {
      ...
      if (response.ok) {
        toast.success('🗑️ Lead deleted successfully!', {
          id: deleteToast  // Replace loading toast
        })
        router.push('/buyers')
      } else {
        // Handle different delete error types
        if (response.status === 403) {
          toast.error('❌ You are not authorized to delete this lead!', {
            id: deleteToast,
            duration: 5000  // Show longer for important security message
          })
        } else if (response.status === 404) {
          toast.error('❌ Lead not found.', { id: deleteToast })
        } else {
          toast.error('❌ Failed to delete lead: ' + (result.error || 'Unknown error'), {
            id: deleteToast
          })
        }
      }
